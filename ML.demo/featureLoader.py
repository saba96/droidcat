...
def malwareCategorize(resultFile):
    ret=dict()
    for mp in file(resultFile).readlines():
        mp = mp.lstrip().rstrip()
        apkfam = string.split(mp)
        ret[ apkfam[0] ] = [apkfam[1], dict()]
...
def getTrainingData(dichotomous=False, \
    ...
    malFam = malwareCategorize(malwareLabelsFile)
