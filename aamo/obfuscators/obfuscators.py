...
def backsmali(sample_tf_dir, sample_file_name):  # Backsmali an apk file
    try:
        ...
        #popen('apktool/apktool d --force --no-debug-info ' + sample_file_name + ' ' + sample_tf_dir + '/app')
        popen('apktool d --force --no-debug-info ' + sample_file_name + ' -o ' + sample_tf_dir + '/app')
...
def smali(sample_tf_dir, sample_file_name):  # Smali an apk file
    try:
        ...
        #print('apktool/apktool b --force-all ' + sample_tf_dir + '/app' + ' ' + sample_file_name)
        #popen('apktool/apktool b --force-all ' + sample_tf_dir + '/app' + ' ' + sample_file_name)
...
def sign_apk(sample_file_name):  # Sign an apk file with a SHA1 key
    try:
        ...
        #popen('/opt/jdk1.8.0_101/bin/jarsigner -sigalg MD5withRSA -digestalg SHA1 -keystore ' + obfuscator_resource_dir + '/resignKey.keystore -storepass resignKey ' + sample_file_name + ' resignKey')
    except OSError as ex:
...
def apply_dir(orgApkDir, obfuscator_to_apply, mode=0, retry=0):
    try:
        #obfuscate_sample(adam_base_dir + 'input/' + filename, obfuscator_to_apply, adam_base_dir + 'temp/' + filename[:-4])
        #sys.exit(0)

        for apk in os.listdir(orgApkDir):
            if not apk.endswith('.apk'):
                continue
            #obfuscate_sample(adam_base_dir + 'input/' + filename, obfuscator_to_apply, adam_base_dir + 'temp/' + filename[:-4])
            obfuscate_sample(orgApkDir+'/'+apk, obfuscator_to_apply, base_dir + 'temp/' + apk[:-4])
        sys.exit(0)
...
'''
# the following obfuscators have been made working okay
